...
class Sizhaiwang:
    ...
    # 登录
    def login(self):
        url = 'https://www.szhai.me/wp-json/jwt-auth/v1/token'
        headers = {
            'referer': 'https://www.szhai.me/',
            'user-agent': self.ua
        }
        data = {
    ...
    def sign(self):
        url = 'https://www.szhai.me/wp-json/b2/v1/userMission'
        result = session.post(url, headers=self.headers)
        print(result.text)

    # 签到
    def get_task_data(self):
        url = 'https://www.szhai.me/wp-json/b2/v1/getTaskData'
        result = session.post(url, headers=self.headers)
        result = json.loads(result.text)
        return result
    ...
    # 自动回复4个帖子
    def comment(self):
        url = 'https://www.szhai.me/wp-json/b2/v1/commentSubmit'
        comment_list = ['感谢楼主分享', '好人一生平安', '楼主幸苦了', '终于找到了', '感谢大佬分享']
        for i in range(4):
            post_result = self.get_task_data()
            post_id = post_result['task']['task_comment']['url'][21:-5]
            data = {
    ...
    def follow(self):
        ...
        reg = r'v-if="follow\[(\d+)\]'
        page_max_reg = r'/(\d+) 页'
        page_url = 'https://www.szhai.me/?s=&type=user'
        result = session.get(page_url, self.headers)
        result = re.findall(page_max_reg, result.text)
        page_max = int(result[0])
        follow_url = 'https://www.szhai.me/wp-json/b2/v1/AuthorFollow'
        check_url = 'https://www.szhai.me/wp-json/b2/v1/checkFollowByids'
        for page in range(page_max):
            page += 1
            user_info_url = 'https://www.szhai.me/page/' + str(page) + '?s&type=user'
